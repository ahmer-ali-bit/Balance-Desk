...
class FirestoreRESTClient {
  ...
  static Future<Map<String, dynamic>?> getDocument(String collection, String docId) async {
    try {
      ...
      debugPrint('REST getDocument failed [${response.statusCode}]: ${response.body}');
      return null;
    } catch (e) {
  ...
  static Future<bool> setDocument(String collection, String docId, Map<String, dynamic> data) async {
    try {
      ...
      debugPrint('REST setDocument failed [${response.statusCode}]: ${response.body}');
      return false;
    } catch (e) {
  ...
  static Future<bool> updateDocument(String collection, String docId, Map<String, dynamic> data) async {
    try {
      ...
      final url = Uri.parse('$_baseUrl/$collection/$docId?$updateMask&key=$_apiKey');
      final body = json.encode({'fields': _toFirestoreFields(data)});
      final response = await http.patch(url, body: body);
      if (response.statusCode == 200) return true;

      debugPrint('REST updateDocument failed [${response.statusCode}]: ${response.body}');
      return false;
    } catch (e) {
  ...
  static Future<List<Map<String, dynamic>>> getCollection(String collection, {String? whereField, String? isEqualTo}) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final results = documents.map((d) => _simplifyDocument(d)).whereType<Map<String, dynamic>>().toList();
      ...
      debugPrint('REST getCollection failed [${response.statusCode}]: ${response.body}');
      return [];
    } catch (e) {
  ...
  static Map<String, dynamic> _toFirestoreFields(Map<String, dynamic> data) {
    return data.map((key, value) {
      ...
      if (value is Map<String, dynamic>) {
        return MapEntry(key, {'mapValue': {'fields': _toFirestoreFields(value)}});
      }
